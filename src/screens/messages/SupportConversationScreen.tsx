...
export default function SupportConversationScreen() {
  ...
  // Use nextPublicSequence - 1 (the server's authoritative last-message index) rather
  // than the highest locally-loaded sequence, which may be lower due to pagination.
  useEffect(() => {
    if (!isFocused || !isActive || !conversation) return;
    const unread =
      conversation.unread_count !== undefined
        ? conversation.unread_count
        : Math.max(0, conversation.next_public_sequence - 1 - conversation.user_last_read_sequence);
    if (unread <= 0) return;
