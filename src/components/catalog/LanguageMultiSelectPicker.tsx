...
import { useLanguagesCatalog } from '@/hooks/catalog/useLanguagesCatalog';
...
type ListItem = LanguageOption | SepItem;

function isSeparator(item: ListItem): item is SepItem {
  return 'isSeparator' in item;
}

function buildList(items: LanguageOption[]): ListItem[] {
...
type Props = {
  ...
  onChange: (items: LanguageOption[]) => void;
  placeholder?: string;
  accentColor?: string;
  textColor?: string;
  mutedColor?: string;
...
export function LanguageMultiSelectPicker({
  selected,
  onChange,
  ...
  const { data, isLoading } = useLanguagesCatalog({
  ...
  const isSelected = useCallback(
    (item: LanguageOption) => selected.some((s) => s.id === item.id),
    [selected],
  );
  ...
  return (
    <>
      ...
      <Pressable
        ...
        accessibilityLabel={`Languages: ${displayLabel}`}
        style={{
          flexDirection: 'row',
          alignItems: 'center',
          borderWidth: 1,
      ...
      <Modal visible={open} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setOpen(false)}>
        <View style={{ flex: 1, backgroundColor: surfaceColor, paddingTop: top }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingTop: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: borderColor, gap: 12 }}>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 17, fontWeight: '700', color: textColor }}>Select Languages</Text>
              {selectedCountryName && (
                <Text style={{ fontSize: 12, color: mutedColor, marginTop: 2 }}>{selectedCountryName}</Text>
              )}
            </View>
            <Text style={{ fontSize: 12, color: mutedColor }}>{selected.length}/{MAX_LANGUAGES}</Text>
          ...
          {isLoading ? (
            ...
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              <Ionicons name="language-outline" size={40} color={mutedColor} />
              <Text style={{ color: mutedColor, fontSize: 14 }}>No languages found</Text>
            </View>
          ) : (
            <FlatList
              data={listItems}
              ...
              renderItem={({ item }) => {
                ...
                const disabled = !sel && selected.length >= MAX_LANGUAGES;
                return (
                  <Pressable
                  ...
                  >
                    <View style={{ flex: 1 }}>
                      ...
                      {item.native_name && item.native_name !== item.name && (
                        <Text style={{ fontSize: 12, color: mutedColor, marginTop: 1 }}>{item.native_name}</Text>
                      )}
                    </View>
