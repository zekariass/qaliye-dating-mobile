...
export function isImageAttachment(att: ChatAttachment): boolean {
  if (att.attachmentType === 'IMAGE') return true;
...
export function ChatImageAttachment({
  ...
  const [resolvedUri, setResolvedUri] = useState<string | null>(attachment.downloadUrl);
  const [urlFetching, setUrlFetching] = useState(!attachment.downloadUrl);

  useEffect(() => {
    if (attachment.downloadUrl) {
      setResolvedUri(attachment.downloadUrl);
    ...
    setUrlFetching(true);
    setError(false);
    refreshAttachmentSignedUrl(attachment.id)
      .then((dto) => {
        if (cancelled) return;
        if (dto.download_url) {
          setResolvedUri(dto.download_url);
        } else {
          setError(true);
        }
      })
      ...
      .finally(() => {
        if (!cancelled) setUrlFetching(false);
      });
    return () => { cancelled = true; };
  }, [attachment.id, attachment.downloadUrl]);

  const uri = resolvedUri;
  if (!uri && !urlFetching && !error) return null;
