...
export type ChatVoiceRecorderStatus =
  | 'idle'
...
export interface ChatVoiceRecorderError {
  code:
    | 'PERMISSION_DENIED'
...
const VOICE_FILE_NAME = 'chat-voice.m4a';
...
export function useChatVoiceRecorder() {
  ...
  const [status, setStatus] = useState<ChatVoiceRecorderStatus>('idle');
  const [error, setError] = useState<ChatVoiceRecorderError | null>(null);
  const [recording, setRecording] = useState<ChatVoiceRecordingResult | null>(null);
  ...
  const requestPermission = useCallback(async (): Promise<boolean> => {
    if (!isAudioAvailable) {
      ...
      const unavailableError: ChatVoiceRecorderError = {
      ...
      maxDurationTimerRef.current = setTimeout(() => {
        stopRecording().catch(() => {
          setError({
            ...
            message: `Recording limit is ${CHAT_VOICE_MAX_DURATION_SECONDS} seconds.`,
          });
        });
  ...
  const stopRecording = useCallback(async (): Promise<ChatVoiceRecordingResult | null> => {
    clearMaxDurationTimer();
    ...
    try {
      ...
      const result: ChatVoiceRecordingResult = {
        uri,
        durationMs,
  ...
  const validateRecording = useCallback(
    (fileSizeBytes: number): boolean => {
      if (fileSizeBytes > CHAT_MAX_VOICE_FILE_SIZE) {
        setError({
          code: 'FILE_TOO_LARGE',
          message: `Voice file is too large. Maximum size is ${Math.round(CHAT_MAX_VOICE_FILE_SIZE / 1024 / 1024)} MB.`,
        });
        setStatus('error');
        return false;
      }
